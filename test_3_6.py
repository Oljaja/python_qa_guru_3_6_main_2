...
def test_csv():
    with zipfile.ZipFile(path_zip) as myzip:
        ...
        with open(csv_f) as csv_file:
            ...
            for line_no, line in enumerate(table, 2):
                if line_no == 1:
                    assert 'Сергеев' in line[2]
        os.remove(csv_f)


def test_xlsx():
    with zipfile.ZipFile(path_zip) as myzip:
        xlsx_f = myzip.extract('file_example.xlsx')
        workbook = load_workbook(xlsx_f)
        sheet = workbook.active
        sheet = sheet.cell(row=8, column=3).value
        assert 'Hurn' in sheet
        os.remove(xlsx_f)
